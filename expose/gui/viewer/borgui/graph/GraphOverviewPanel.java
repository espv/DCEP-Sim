...
/**
 * GraphOverviewPanel provides a 'birds-eye' view of the active document.
 *
 */
public class GraphOverviewPanel extends JPanel
        implements ComponentListener, GraphModelListener, GraphLayoutCacheListener, PropertyChangeListener {
     ...
     */
    public GraphOverviewPanel(JGraph g, JViewport viewPort, boolean flag) {
        originalGraph = g;
        v = new PannerViewfinder(this, viewPort);
        GraphLayoutCache view = new ViewRedirector(g.getGraphLayoutCache());
        graph = new BorealisGraph();
        graph.setModel(g.getModel());
     ...
     * Must use the {@link #createOverviewPanel} method to create a new instance.
     */
    protected GraphOverviewPanel(JGraph g, JViewport viewPort) {
        originalGraph = g;
        v = new PannerViewfinder(this, viewPort);
        GraphLayoutCache view = new ViewRedirector(g.getGraphLayoutCache());
        graph = new BorealisGraph();
    ...
    /** GPOverviewPanel factory that returns instance with small inset as a buffer.*/
        /*
        public static JPanel createGraphOverviewPanel(JGraph g, JViewport view){
                ...
                panelWithInternalOffset.add(new GraphOverviewPanel(g,view), BorderLayout.CENTER);
                return panelWithInternalOffset;
        }*/
    
    public static GraphOverviewPanelWrapper createGraphOverviewPanel(JGraph g, JViewport view){
        GraphOverviewPanel over = new GraphOverviewPanel(g, view);
        GraphOverviewPanelWrapper panelWithInternalOffset = new GraphOverviewPanelWrapper(over);
        panelWithInternalOffset.setLayout(new BorderLayout());
        panelWithInternalOffset.setBorder(new MatteBorder(PANEL_BUFFER,PANEL_BUFFER,PANEL_BUFFER,PANEL_BUFFER,Color.WHITE));
        panelWithInternalOffset.add(over, BorderLayout.CENTER);
        return panelWithInternalOffset;
    }
    ...
    private class PannerViewfinder  implements MouseListener, MouseMotionListener, ChangeListener {
        ...
        public void mouseClicked(MouseEvent e) {
            ...
            if (!isContained(e)) {
                isActive = true;
                int x = Math.min(getWidth() - pannerViewRectangle.width,
                        Math.max(0, e.getX() - pannerViewRectangle.width/2));
                int y = Math.min(getHeight() - pannerViewRectangle.height,
                        Math.max(0, e.getY() - pannerViewRectangle.height/2));
//				pannerViewRectangle.setLocation(e.getPoint());
                pannerViewRectangle.setLocation(x, y);
