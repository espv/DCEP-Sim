...
public class NetworkTreeModel  implements TreeModel
{
    ...
    protected final static String DATASET_ROOT  = new String("Datasets:");
    protected final static String EXPERIMENT_ROOT  = new String("Experiments:");
    protected final static String NETWORKPARAMETER_ROOT  = new String("Network-params:");
    protected final static String PLOT_ROOT  = new String("Plots:");
...
//////////////// TreeModel interface implementation ///////////////////////
    ...
    protected Vector<Object> getChildVector(Object parent)
    {
        ...
        if (parent instanceof NetworkElement)
        {
            ...
            children.add(DATASET_ROOT);
            children.add(EXPERIMENT_ROOT);
            children.add(NETWORKPARAMETER_ROOT);
            children.add(PLOT_ROOT);
            // query node
        }
        ...
        else if (parent instanceof String)
        {
            ...
            else if (node.equals(DATASET_ROOT))
            {
                //children.addAll(m_doc.getAllQueries());
                List   datasets = m_doc.getAllDatasets();

                for(Iterator i = datasets.iterator(); i.hasNext();)
                {
                    children.add((DatasetElement)i.next());
                }
            }
            else if (node.equals(EXPERIMENT_ROOT))
            {
                //children.addAll(m_doc.getAllQueries());
                List   experiments = m_doc.getAllExperiments();

                children.addAll(experiments);
            }
            else if (node.equals(NETWORKPARAMETER_ROOT))
            {
                //children.addAll(m_doc.getAllQueries());
                NetworkParameterElement networkParameterElement = m_doc.getNetworkParameters();
                if (networkParameterElement == null) {
                    networkParameterElement = new NetworkParameterElement("1000000", "1000000", "0", "0");
                    m_doc.setNetworkParameters(networkParameterElement);
                }

                if (children.isEmpty()) {
                    children.add(networkParameterElement);
                } else {
                    children.set(0, networkParameterElement);
                }
            }
            else if (node.equals(PLOT_ROOT))
            {
                //children.addAll(m_doc.getAllQueries());
                List   plots = m_doc.getAllPlots();

                children.addAll(plots);
            }
        }
